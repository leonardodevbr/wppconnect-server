...
routes.post('/api/:session/webhook', async (req, res) => {
  try {
    ...
    // Salvar webhook no MongoDB usando importação dinâmica
    const Token = require('../util/tokenStore/model/token').default;
...
routes.get('/api/:session/config', async (req, res) => {
  try {
    const { session } = req.params;
    
    // Buscar configurações no MongoDB usando importação dinâmica
    const Token = require('../util/tokenStore/model/token').default;
    const config = await Token.findOne({ sessionName: session });
...
// Listar instâncias (simplificado)
...
// Rota para verificar status de recuperação das sessões
routes.get(
  '/api/:session/recovery-status',
  verifyToken,
  (req: any, res: any) => {
    const sessionName = req.params.session;
    const retryCount = sessionRecoveryManager.getRetryCount(sessionName);
    
    res.json({
      session: sessionName,
      retryCount: retryCount,
      maxRetries: 10,
      status: retryCount === 0 ? 'stable' : 'recovering',
...
// Carrosséis
routes.post(
  '/api/:session/send-carousel',
  verifyToken,
  statusConnection,
  CarouselController.sendCarousel
);
routes.post(
  '/api/:session/send-carousel-images',
  verifyToken,
  statusConnection,
  CarouselController.sendCarouselWithImages
);
...
routes.delete(
  '/api/:session/clear-queue',
  verifyToken,
  clearQueue
);
routes.delete(
  '/api/:session/queue/:messageId',
  verifyToken,
  removeMessage
);
routes.get(
  '/api/:secretkey/all-queues',
  getAllQueues
);
